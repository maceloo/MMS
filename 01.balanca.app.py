...
class AfericaoBalancaApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Registro de Aferição de Balanças de Laboratório")

        # Definindo cores
        self.cor_fundo = "#f0f0f0"
        self.cor_titulo = "#4b0082"
        self.cor_label_frame = "#dcdcdc"
        self.cor_botao = "#4682b4"
        ...
        tk.Label(self.frame_pesos, text="Peso 01 (g):", bg=self.cor_label_frame).grid(row=0, column=0)
        tk.Entry(self.frame_pesos, textvariable=self.peso1, width=20).grid(row=0, column=1)
        
        tk.Label(self.frame_pesos, text="Peso 02 (g):", bg=self.cor_label_frame).grid(row=1, column=0)
        tk.Entry(self.frame_pesos, textvariable=self.peso2, width=20).grid(row=1, column=1)
        
        tk.Label(self.frame_pesos, text="Peso 03 (g):", bg=self.cor_label_frame).grid(row=2, column=0)
        ...
        self.botao_calcular = tk.Button(self.frame_resultados, text="Calcular Média e Limite de Erro", command=self.calcular_media, bg=self.cor_botao, fg="white")
        ...
        tk.Label(self.frame_resultados, text="Limite de Erro (g):", bg=self.cor_label_frame).grid(row=2, column=0)
        ...
        self.botao_salvar = tk.Button(self.frame_principal, text="Salvar em Excel", command=self.salvar_excel, bg=self.cor_botao, fg="white")
    ...
    def salvar_excel(self):
        try:
            ...
            dados = [
                ...
                ["Limite de Erro (g)", self.limite_erro.get()]
            ]
